...
function evil_core_charge(keys)
    print("[abilities_simple] evil_core_charge")
    local caster = keys.caster
    local time = 15
    ...
    Timers:CreateTimer(0.03,function()
        ...
        Timers:CreateTimer(0.25,function()
			...
			if not caster:IsAlive() and not caster.hasSummonedAsura then
				...
				spawn_unit_arround( caster, "npc_dota_boss_juggernaut", 50, 1, nil, true)
				return
			end
			local weakness = true
			...
			if weakness == true then 
				if caster.weakness == false then
                    ...
                    Say(nil, "Evil core is now weak !", true)
...
function miniboss_reflect_hit(keys)
...
function boss_hell_guardian_death(keys)
    gold_rain(keys.caster,10000,10000,25)
end
...
function projectile_dark_orbs( event )
    ...
    local origin = caster:GetAbsOrigin()
    local projectile_count = 150 --ability:GetLevelSpecialValueFor("projectile_count", ability:GetLevel()-1) -- If you want to make it more powerful with levels
    local speed = 1300
    ...
    Timers:CreateTimer(1.0,function()
        ...
        for i=-720,720,(1440/projectile_count) do
            ...
            Timers:CreateTimer(time, function()
                EmitSoundOn("Hero_Puck.Illusory_Orb", caster) --Add a sound if you wish!
...
function projectile_death_orbs_hit( event )
    ...
    if target:GetHealth() <= target:GetMaxHealth()/4 then 
        ...
        elseif GetMapName() == "epic_boss_fight_hard"then
            target:SetHealth( math.ceil(target:GetHealth()*0.7 + 1 ) )
        else 
...
function hell_tempest_hit( event )
    ...
    if target.InWater ~= true then
        if target:GetUnitName()~="npc_dota_courier" and target:GetUnitName()~="npc_dota_flying_courier" then
            event.ability:DealDamage( event.caster, target, 200000, {damage_type = DAMAGE_TYPE_MAGICAL} )
        end
    end
end
...
function doom_raze( event )
    ...
    Timers:CreateTimer(0.25,function() 
            ...
            location = location - rv * 2500 + fv * 400
            if created_line <= 10 then
                return 0.5
            end
        end)
...
function doom_bringer_boss( event )
    ...
    for _, target in pairs( units ) do
        target:AddNewModifier(caster, nil, "modifier_imba_doom_bringer_doom_enemies",{duration = 10})
    end

    -- for _, target in pairs( units ) do
    --     local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_doom_bringer/doom_bringer_doom_ring.vpcf",PATTACH_POINT_FOLLOW,target)
    --     target:AddNewModifier(caster, nil, "modifier_imba_doom_bringer_doom_enemies",{duration = 15})
    --     Timers:CreateTimer(1.0,function()
